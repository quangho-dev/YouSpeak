...
export const getCurrentProfile = () => async (dispatch) => {
  try {
    const res = await api.get('/profileStudent/me')
...
export const createOrUpdateProfile = (
  ...
  try {
    const res = await api.post('/profileStudent', formData)
...
export const deleteAccount = () => async (dispatch) => {
  if (window.confirm('Are you sure? This can NOT be undone!')) {
    try {
      await api.delete('/profileStudent')
...
export const getProfileStudentById = (studentId) => async (dispatch) => {
  try {
    const res = await api.get(`/profileStudent/${studentId}`)
